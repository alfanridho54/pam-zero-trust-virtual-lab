    ...
    <main class="main">
        <header class="header">
            <div>
                ...
                <div class="page-subtitle">Monitoring lab, VM praktikum, dan aktivitas pengguna mock Proxmox.</div>
            </div>
        ...
        <div class="content">
            ...
            @if (in_array($section, ['overview', 'vms'], true))
                <section class="card section">
                    <div class="section-head">
                        <div>
                            <h2 class="section-title">Virtual Machine</h2>
                            <div class="section-note">Daftar VM dengan owner, resource, status, dan aksi simulasi.</div>
                        </div>
                    </div>
                    <div class="table-wrap">
                        <table>
                            ...
                            @forelse ($vms as $vm)
                                <tr>
                                    ...
                                    <td>{{ $vm->user?->name ?? '-' }}<div class="muted">owner_id={{ $vm->user_id }}</div></td>
                                    <td>{{ $vm->labTemplate?->name ?? 'Lab Pribadi' }}</td>
                                    <td>{{ $vm->cpu_cores }} core</td>
                                    <td>{{ $vm->memory_mb }} MB</td>
                                    <td>{{ $vm->disk_gb }} GB</td>
                                    <td><span class="badge {{ $statusClass($vm) }}">{{ $statusLabel($vm) }}</span></td>
                                    <td>
                                        <div class="actions">
                                            <form method="POST" action="{{ route('dashboard.simulate.vm.resources', $vm) }}">
                                                @csrf
                                                <button class="btn btn-soft" type="submit" @disabled($vm->trashed())>Edit resource VM</button>
                                            </form>
                                            <form method="POST" action="{{ route('dashboard.simulate.vm.delete', $vm) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-danger" type="submit" @disabled($vm->trashed())>Soft delete VM</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="muted">Belum ada VM. Gunakan tombol Create Docker Lab untuk owner_id=3.</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
